...
Laplacian<Dim, Order>::Laplacian(Laplacian &&l) noexcept
    : specs_(std::move(l.specs_)), mesh_(std::move(l.mesh_)),
      Xh_(std::move(l.Xh_)), u_(std::move(l.u_)), v_(std::move(l.v_)),
      a_(std::move(l.a_)), at_(std::move(l.at_)), l_(std::move(l.l_)),
      lt_(std::move(l.lt_)), bdf_(std::move(l.bdf_)), e_(std::move(l.e_)),
      meas_(std::move(l.meas_)) 
{
  // Optionally, handle the moved-from state if necessary
}
template <int Dim, int Order>
Laplacian<Dim, Order> &Laplacian<Dim, Order>::operator=(Laplacian const &l) 
{
  if (this != &l) 
  {
    specs_ = l.specs_;
