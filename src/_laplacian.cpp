...
laplacian_inst( py::module &m )
{
    ...
    py::class_<Laplacian<Dim, Order>>( m, fmt::format( "Laplacian{}DP{}", Dim, Order ).c_str() )
        ...
        .def( py::init<const Laplacian<Dim, Order>&>(), "Copy constructor")
        //.def( py::init<Laplacian<Dim, Order>&&>(), "Move constructor")
        .def( "__assign__", &Laplacian<Dim, Order>::operator=, "Copy assignment operator")
