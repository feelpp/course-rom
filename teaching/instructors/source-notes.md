...
## Overview of general outputs, nonsymmetry and parabolic prediction

`rom::certification/extensions.adoc` is a concise teaching bridge, not a full conversion of either advanced deck. Sources reviewed: public `lecture-rbobm-beamer-l4.pdf` (especially residual/general-output discussion, pp. 12–18, and the primal–dual section), and `lecture-rbobm-beamer-parabolic.pdf` (affine mass/stiffness, backward Euler, reduced cost, trajectory norms and dual extension). The linked Grepl–Patera M2AN 2005 article is further reading for detailed transient output certification.

The page uses the positive dual convention `A^T z = ell`, so the corrected output adds the primal residual evaluated on the approximate dual. Its product bound is derived using the primal coercivity bound; nonsymmetry does not remove coercivity automatically. The discrete transient bound is presented for fixed symmetric positive-definite mass, a time-independent coercive operator per parameter query, backward Euler and a consistent full/reduced time grid. It explicitly includes initial projection error. The residual uses the stiffness-based state metric, while final-time output control uses the mass-dual norm. No steady compliant identity or instantaneous residual product is claimed for general transient outputs.

The 2D thermal route uses the already supplied FE data and fixed mass, with positive conductivity/Robin terms. Taking a positive reference Robin coefficient gives a simple minimum-coefficient coercivity lower bound. Training snapshots include parameter/time sampling and time-quadrature weights; the POD tail remains a training-projection statement. The page outlines prediction-first work before the existing EnKF practical rather than silently adding a new graded assignment or replacing the current session schedule.

`verify-mathematical-notes.py` now checks nonsymmetric primal/dual residual identities and product bounds with distinct bases and a non-diagonal metric, the affine cross-matrix output correction, accumulated backward-Euler error bounds and final output bounds with nonzero initial error, reduced dissipation during cooling, failure of reduced invertibility without coercivity, and the positive-affine minimum-coefficient lower bound. These are finite-dimensional verification examples, not a validation of a new full thermal notebook.
